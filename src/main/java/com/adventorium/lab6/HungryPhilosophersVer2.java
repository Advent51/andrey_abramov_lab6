...
public class HungryPhilosophersVer2 {
    ...
    private class ChopStick {
        ...
        volatile private Integer owner = -1;

        ChopStick(int id) {
            this.id = id;
        }

        void pickUp(int owner_id) {
            this.owner = owner_id;
        }
            ...
            try {
                while (!isTummyFull) {
                    ...
                    synchronized (leftChopStick) {
                        if (leftChopStick.owner != -1) {
                            leftChopStick.wait(randomGenerator.nextInt(100));
                        } else {
                            leftChopStick.pickUp(this.id);
                            synchronized (rightChopStick) {
                                if (rightChopStick.owner != -1) {
                                    rightChopStick.wait(randomGenerator.nextInt(100));
                                } else {
                                    rightChopStick.pickUp(this.id);
                                    eat();
                                    rightChopStick.putDown();
                                    rightChopStick.notifyAll();
                                }
                            }
                            leftChopStick.putDown();
                            leftChopStick.notifyAll();
                        }
                    }
                }
            } catch (Exception e) {
